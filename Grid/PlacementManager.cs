...
public class PlacementManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Q) && GridManager.tool != GridManager.SELECT) {
            animationTime = 0;
            if ((int)dir == 0)
            {
                dir = (Direction_e)3;
        ...
        if (Input.GetKeyDown(KeyCode.E) && GridManager.tool != GridManager.SELECT) {
            animationTime = 0;
            if ((int)dir == 3)
            {
                dir = (Direction_e)0;
        ...
        if (Input.GetMouseButton(0)) {
            ...
            if (GridManager.tool == GridManager.DRAG || GridManager.tool == GridManager.SELECT)
                return;

            if (GridManager.tool == GridManager.PLACEMENT)
            {
            ...
            {
                if (CellFunctions.cellGrid[x, y].cellType != (int)GridManager.tool || CellFunctions.cellGrid[x, y].getDirection() != (Direction_e)dir) {
            ...
            Cell cell = GridManager.instance.SpawnCell((int)GridManager.tool, new Vector2(x,y), dir, false);
            GridManager.hasSaved = false;
        }

        if (Input.GetMouseButton(1) && GridManager.tool != GridManager.SELECT)
        {
            if (!GridManager.clean)
                return;
            if (x < 0 || y < 0)
