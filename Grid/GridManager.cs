...
public class GridManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        cellPrefabs = GameObject.Find("CellLoader").GetComponent<CellLoader>().cellPrefabsList.ToArray(); // 12 under normal
        PLACEMENT = cellPrefabs.Length;
        DRAG = cellPrefabs.Length + 1;
        SELECT = cellPrefabs.Length + 2; // does it make sense? no. does it work? yes.
        tool = DRAG;
        ...
        foreach (int cellType in CellFunctions.cellUpdateOrder) {
    ...
    public Cell SpawnCell(int cellType, Vector2 position, Direction_e rotation, bool generated) {
        print(cellType);
        GameObject cellObj = Instantiate(this.cellPrefabs[cellType]);
        Cell cell = cellObj.GetComponent<Cell>();
        if (cellType > 8) { cellObj.GetComponent<LuaCell>().Init(cellType); }
    ...
    private void UpdateTicked(int cellType) {
    ...
    private void UpdateTracked(int cellType, Direction_e dir) {
    ...
    private void MetaSortTrackedCells(int cellType)
    {
    ...
    private void SortTrackedCells(int cellType, Direction_e dir) {
        int steppedCellId = CellFunctions.GetSteppedCellId(cellType);
    ...
    private void Update()
    {
        ...
        if (timeSinceLastUpdate > animationLength && (playSimulation || stepSimulation)) {
            ...
            if (subTick)
            {
                if (CellFunctions.GetUpdateType(cellUpdateIndex).Equals(CellUpdateType_e.TRACKED))
                {
                    if (rotationUpdateIndex == 0)
                    {
                        MetaSortTrackedCells(cellUpdateIndex);
                    }
                    SortTrackedCells(cellUpdateIndex, CellFunctions.directionUpdateOrder[rotationUpdateIndex]);
                    UpdateTracked(cellUpdateIndex, CellFunctions.directionUpdateOrder[rotationUpdateIndex]);
                    rotationUpdateIndex++;
                    ...
                    UpdateTicked(cellUpdateIndex);
                    cellUpdateIndex++;
                }
                if (cellUpdateIndex >= CellFunctions.cellUpdateOrder.ToArray().Length) {
                    cellUpdateIndex = 0;
                }
            }
            else {
